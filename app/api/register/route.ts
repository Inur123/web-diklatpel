...
export async function POST(request: Request) {
  try {
    ...
    for (const field of REQUIRED_FILE_FIELDS) {
      ...
      // 1. Wajib PDF saja (essay, formulir)
      if (["essay", "formulir"].includes(field)) {
      ...
      // 3. Wajib Gambar / PDF (sertifikatMakesta, rekomendasi, sertifikatLakmud, ktpKta, paktaIntegritas, buktiBayar)
      if (["sertifikatMakesta", "rekomendasi", "sertifikatLakmud", "ktpKta", "paktaIntegritas", "buktiBayar"].includes(field)) {
        const isPdf = fileName.endsWith(".pdf");
        const isImage = fileName.endsWith(".png") || fileName.endsWith(".jpg") || fileName.endsWith(".jpeg");
        if (!isPdf && !isImage) {
          return NextResponse.json(
            { error: `Berkas "${field}" wajib memiliki format PDF atau gambar (PNG / JPG) saja.` },
            { status: 400 }
          );
        }
      }
    ...
    const FILE_NAME_MAPPING: Record<string, string> = {
      sertifikatMakesta: "sertifikatDiklatama",
      sertifikatLakmud: "suratKeteranganSehat",
      ktpKta: "suratIzinOrangTua",
      rekomendasi: "suratRekomendasi",
      essay: "karyaTulisEsai",
      fotoFormal: "fotoFormalPdl",
      formulir: "formulirPendaftaran",
    ...
    const uploadResults = await Promise.all(
      Object.entries(fileEntries).map(async ([fieldKey, file]) => {
        ...
        const customFileName = FILE_NAME_MAPPING[fieldKey] || fieldKey;
        const r2Key = `registrations/${sanitizedName}/${customFileName}.${ext}`;
