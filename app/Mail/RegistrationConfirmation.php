...
class RegistrationConfirmation extends Mailable implements ShouldQueue
{
    ...
    {
        return new Envelope(
            subject: 'Konfirmasi Pendaftaran DIKLATPEL 2026 — '.$this->registration->name,
        );
    }
    ...
    public function build(): self
    {
        return $this->withSymfonyMessage(function (Email $message) {
            $logoPath = public_path('images/logo-putih-2-diklatpel.webp');
            if (file_exists($logoPath)) {
                $message->embedFromPath($logoPath, 'logo-putih-2-diklatpel.webp');
            }
        });
    }
}
